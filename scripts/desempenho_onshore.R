...
data %<>% 
  ...
  mutate(name = case_when(name == "IDA-IPCA Infraestrutura"~"IDA-Infra", 
         ...
         acumulado_12_meses = (zoo::rollapply(1 + var, 
                                              width = 252, 
                                              FUN = prod, 
                                              align = 'right', 
                                              fill = NA) - 1)) %>% 
...
for (i in retorno) {
  ...
  g = data %>% 
    ...
    scale_colour_manual(
      ...
      labels = c(
        "Ibovespa" = paste0("Ibovespa: ", round(lst_dt$value[which(lst_dt$name == "Ibovespa" & lst_dt$retorno == i)],2), "%"),
        "CDI" = paste0("CDI: ", round(lst_dt$value[which(lst_dt$name == "CDI" & lst_dt$retorno == i)],2), "%"),
        "IDA-DI" = paste0("IDA-DI: ", round(lst_dt$value[which(lst_dt$name == "IDA-DI" & lst_dt$retorno == i)],2), "%"),
        "IHFA" = paste0("IHFA: ", round(lst_dt$value[which(lst_dt$name == "IHFA" & lst_dt$retorno == i)],2), "%"),
        "IFIX" = paste0("IFIX: ", round(lst_dt$value[which(lst_dt$name == "IFIX" & lst_dt$retorno == i)],2), "%"),
        "SMLL" = paste0("SMLL: ", round(lst_dt$value[which(lst_dt$name == "SMLL" & lst_dt$retorno == i)],2), "%"),
        "IMA-B" = paste0("IMA-B: ", round(lst_dt$value[which(lst_dt$name == "IMA-B" & lst_dt$retorno == i)],2), "%"),
        "IMA-B 5" = paste0("IMA-B 5: ", round(lst_dt$value[which(lst_dt$name == "IMA-B 5" & lst_dt$retorno == i)],2), "%"),
        "IRF-M" = paste0("IRF-M: ", round(lst_dt$value[which(lst_dt$name == "IRF-M" & lst_dt$retorno == i)],2), "%"),
        "Dólar" = paste0("Dólar: ", round(lst_dt$value[which(lst_dt$name == "Dólar" & lst_dt$retorno == i)],2), "%")
        )) +
  ...
  ggsave(paste0(i, ".png"), 
         ...
         dpi = 576, 
         # dpi = 800,
         path = paste0(getwd(), "/output/onshore"))
