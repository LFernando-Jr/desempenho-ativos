...
# df <- Reduce(function(x, y) merge(x, y, by = "date", all = TRUE), list(SPX, IXIC, DJI, RUT, AGG, TIP, STIP, SGOV, DXY, DGS2, DGS10, HG, HY))
# df <- df %>% 
...
data %>%
  ...
  scale_colour_manual(values = c("spx" = "#2F47AD",
                      ...
                      labels = c("spx" = paste0("S&P: ", tbl$`Retorno acumulado no ano`[which(tbl$name == "spx")], "%"),
                                 ...
                                 "agg" = paste0("AGG: ", tbl$`Retorno acumulado no ano`[which(tbl$name == "agg")], "%"),
...
data %>%
  ...
  scale_colour_manual(values = c("spx" = "#2F47AD",
                      ...
                      labels = c("spx" = paste0("S&P: ", tbl$`Retorno acumulado no mês`[which(tbl$name == "spx")], "%"),
                                 ...
                                 "agg" = paste0("AGG: ", tbl$`Retorno acumulado no mês`[which(tbl$name == "agg")], "%"),
