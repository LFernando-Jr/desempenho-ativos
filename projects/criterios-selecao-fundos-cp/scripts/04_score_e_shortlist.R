...
ranking_base = metricas_todos_fundos %>%
  ...
  mutate(
    ...
    score_retorno = score_maior_melhor(excesso_cdi_aa),

    # Consistência: 25% do score final.
    score_consistencia = 0.40 *
      score_maior_melhor(hit_rate_mensal) +
    ...
    score_risco = 0.40 *
      score_maior_melhor(max_drawdown_excesso) +
      0.30 * score_maior_melhor(media_tres_piores_meses) +
      0.30 * score_menor_melhor(volatilidade_excesso_aa),

    # Custo: 25% do score final.
    eficiencia_custo = excesso_cdi_aa /
      pmax(taxa_adm_aa, 0.0001),

    score_taxa = score_menor_melhor(taxa_adm_aa),

    score_eficiencia = score_maior_melhor(eficiencia_custo),

    score_custo = 0.60 * score_taxa + 0.40 * score_eficiencia,

    # Diferenciação: 10% do score final.
    # Menor correlação com os pares recebe maior nota.
    score_diferenciacao = 0.60 *
      score_menor_melhor(correlacao_media_pares) +
      0.40 * score_menor_melhor(correlacao_maxima),

    score_preliminar = 0.20 *
      score_retorno +
...
ranking_fundos = ranking_base %>%
  ...
  mutate(
    ...
    elegivel_shortlist = quartil_score <= 2 &
      is.na(red_flags)
  ) %>%
  group_by(cluster) %>%
  arrange(
  ...
  mutate(
    ranking_no_cluster = row_number(),

    ranking_elegivel_no_cluster = if_else(
      elegivel_shortlist,
      cumsum(elegivel_shortlist),
      NA_integer_
    ),

    shortlist = elegivel_shortlist &
      ranking_elegivel_no_cluster <= MAX_FUNDOS_POR_CLUSTER
  ) %>%
  ungroup() %>%
  mutate(
    ...
    quartil_retorno = ntile(desc(score_retorno), 4),

    quartil_consistencia = ntile(desc(score_consistencia), 4),

    quartil_risco = ntile(desc(score_risco), 4),

    quartil_custo = ntile(desc(score_custo), 4),

    quartil_diferenciacao = ntile(desc(score_diferenciacao), 4)
  ) %>%
  arrange(ranking_geral)

write_excel_csv2(
  ranking_fundos,
  "projects/criterios-selecao-fundos-cp/data/intermediate/ranking_fundos_etapa2_36m.csv"
)

write_rds(
  ranking_fundos,
  "projects/criterios-selecao-fundos-cp/data/intermediate/ranking_fundos_etapa2_36m.rds"
)
