...
# 1. Universo elegível
# ------------------------------------------------------------
# A Etapa 1 já excluiu fundos sem 36 meses completos.
# Aqui fazemos uma validação adicional:
# - sem pendência de revisão;
# - série chegando à data máxima da base.
...
grafico_consistencia <- ggplot(
  ...
  labs(
    ...
    caption = paste(
      ...
      "Fundos com apenas 36 meses possuem uma única janela de 36 meses."
    )
  ) +
...
resumo_etapa2 <- fundos_elegiveis |>
  select(
    nome_xlsx,
    nome_plot,
    nome_quantum,
